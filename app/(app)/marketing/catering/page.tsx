...
import { CATERING_LEADS, type CateringLead } from "@/lib/data/marketing-sample";

export const dynamic = "force-dynamic";

function statusTone(s: CateringLead["status"]): string {
  switch (s) {
    case "inquiry":
...
export default function CateringPage() {
  const total = CATERING_LEADS.length;
  const booked = CATERING_LEADS.filter((l) => l.status === "booked").length;
  const pipeline = CATERING_LEADS.filter((l) => l.status !== "lost").reduce(
    (sum, l) => sum + l.budget,
    0,
  );
  ...
  return (
    <div>
      <PageHeader
        title="Catering"
        description="Office drop-offs, team lunches, community accounts. Repeat business is the goal."
      />

      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Kpi label="Inquiries (mo)" value={String(total)} />
        <Kpi label="Booked" value={String(booked)} />
        <Kpi
          label="Repeat accounts"
          value="—"
          sub="Tracking starts next month"
        />
        <Kpi
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-t border-border">
          ...
          <tbody className="text-sm">
            {CATERING_LEADS.map((l) => (
              <tr
                key={l.company + l.date}
