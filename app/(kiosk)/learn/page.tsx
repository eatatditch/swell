...
export default async function KioskHomePage() {
  ...
  const [overview, announcements] = await Promise.all([
    getStaffTrainingOverview(staff.id),
  ...
  return (
    <>
      <PageHeader
        ...
        description={TRAINING_STAFF_TYPE_LABELS[staff.staff_type]}
        action={
          <Link
            href="/learn/courses"
          ...
          <CardContent>
            {overview.paths.length === 0 ? (
              <EmptyState
                ...
                description="A manager will assign your path. In the meantime, check the course library."
              />
            ) : (
                      ...
                      <ul className="mt-3 space-y-1.5">
                        {courses.map((pc) => {
                          ...
                          return (
                            <li key={pc.id}>
                              <Link
                                href={`/learn/courses/${pc.course.slug}`}
